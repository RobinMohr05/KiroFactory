    ...
    try {
      ...
      if (failed && failed.length > 0) {
        ...
        const failedTasks: typeof parsedTasks = failed.map((f) => {
          const task = f.task as Record<string, unknown>;
          return {
            title: String(task.title ?? ''),
            description: task.description !== undefined ? String(task.description) : undefined,
            priority: Number(task.priority ?? 2),
            type: String(task.type ?? 'feature'),
            files: Array.isArray(task.files) ? task.files as string[] : undefined,
            // Drop dependsOnBatchIndex — indices are stale after the array changed
            dependsOnTaskId: Array.isArray(task.dependsOnTaskId) ? task.dependsOnTaskId as number[] : undefined,
            groupId: task.groupId !== undefined ? String(task.groupId) : undefined,
          };
        });
