...
# DB_SERVER, DB_USER, DB_PASSWORD, JWT_SECRET, ENCRYPTION_KEY
...
log "Resolving Container Apps Environment ID..."
ENV_ID=$(az containerapp env show \
  --name "kirofactory-env" \
  --resource-group "$RESOURCE_GROUP" \
  --query id \
  --output tsv 2>/dev/null || echo "")

if [ -z "$ENV_ID" ]; then
  echo "ERROR: Container Apps Environment 'kirofactory-env' not found."
...
if [ "$WHAT_IF" = true ]; then
  ...
  echo "  Image:     $ACR_LOGIN_SERVER/kirofactory:$IMAGE_TAG"
  echo ""
