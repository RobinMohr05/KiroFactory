...
if [ "$WHAT_IF" = true ]; then
  ...
  echo ""
  echo "  2. Add the static IP to Azure SQL Server firewall rules:"
  echo "     az sql server firewall-rule create \\"
  echo "       --resource-group <sql-rg> --server rm-sandbox \\"
  echo "       --name kirofactory-aca \\"
  echo "       --start-ip-address $(echo "$RESULT" | jq -r '.properties.outputs.staticIp.value') \\"
  echo "       --end-ip-address $(echo "$RESULT" | jq -r '.properties.outputs.staticIp.value')"
  echo ""
  echo "  3. Deploy the Container App (orchestrator):"
  echo "     export DB_SERVER=<server> DB_USER=<user> DB_PASSWORD=<pw>"
