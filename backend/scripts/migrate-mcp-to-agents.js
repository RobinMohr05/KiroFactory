...
function buildAtlassianServer(creds) {
...
function buildAzureDevopsServer(creds) {
...
function buildAwsApiServer(creds) {
...
function buildAwsDocsServer() {
    return { command: "npx", args: ["-y", "@anthropic/aws-docs-mcp-server"], env: {} };
}
function toMcpServerConfig(name, entry) {
    return {
        name,
...
function buildLocalMcpServerEntries(mcpConfig, credentials) {
    const entries = [];
    if (mcpConfig.atlassian) {
        const entry = buildAtlassianServer(credentials);
        if (entry)
            entries.push(toMcpServerConfig("atlassian", entry));
    }
    if (mcpConfig.azureDevops) {
        const entry = buildAzureDevopsServer(credentials);
        if (entry)
            entries.push(toMcpServerConfig("azure-devops", entry));
    }
    if (mcpConfig.awsApi) {
        const entry = buildAwsApiServer(credentials);
        if (entry)
            entries.push(toMcpServerConfig("aws-api", entry));
    }
 ...
 */
export async function resolveEffectiveMcpConfig(session) {
    ...
    if (firstTabId != null) {
        const tabConfig = await readQuery(async (tx) => {
            const result = await tx.run(`MATCH (t:Tab {id: $tabId})-[:HAS_MCP_CONFIG]->(m:McpConfig)
         RETURN m {.*} AS mcpConfig`, { tabId: firstTabId });
            if (result.records.length === 0)
                return null;
            return result.records[0].get("mcpConfig");
        });
        if (tabConfig) {
            config = {
    ...
    const sessionOverride = await readQuery(async (tx) => {
        const result = await tx.run(`MATCH (s:Session {id: $sessionId})-[:HAS_MCP_CONFIG_OVERRIDE]->(m:McpConfig)
       RETURN m {.*} AS mcpConfig`, { sessionId: session.id });
        if (result.records.length === 0)
            return null;
        return result.records[0].get("mcpConfig");
    });
...
async function main() {
    ...
    for (const session of sessions) {
        ...
        try {
            ...
            console.log(`  [SKIP] Session ${session.id} "${session.name}" — failed to decrypt credentials: ${err.message}`);
            skippedCount++;
            continue;
        }
        ...
        const updatedSession = { ...session, mcpServers: updatedMcpServers };
...
//# sourceMappingURL=migrate-mcp-to-agents.js.map