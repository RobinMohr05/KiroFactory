...
export async function initSessions(): Promise<void> {
  ...
  for (const meta of persisted) {
    ...
    const metaWithFlag = meta as Session & { __wasRunning?: boolean };
    if (metaWithFlag.__wasRunning && !meta.cronExpression && !persistedPooledIds.has(meta.id)) {
      toRestart.push(meta.id);
      delete metaWithFlag.__wasRunning;
    }
  }
