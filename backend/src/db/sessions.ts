...
export async function getAllSessionsFromDb(
  ...
  if (userId) {
    result = await pool
      ...
      .query(
        "SELECT * FROM sessions WHERE user_id = @userId ORDER BY pinned DESC, sort_order ASC, created_at DESC"
      );
  } else {
    result = await pool
      .request()
      .query("SELECT * FROM sessions ORDER BY pinned DESC, sort_order ASC, created_at DESC");
  }
...
/**
 * Bulk update sort_order for sessions based on array position.
 * sessionIds[0] gets sort_order=0, sessionIds[1] gets sort_order=1, etc.
 */
...
/**
 * Update the pinned state and sort_order for a session.
 */
