...
import type { Agent, CreateAgentInput, UpdateAgentInput, AgentKind } from "../types.js";
...
function mapRowToAgent(row: Record<string, unknown>): Agent {
  return {
    ...
    kind: ((row.kind as string) || "editor") as AgentKind,
    claimState: (row.claim_state as string) || null,
    workingState: (row.working_state as string) || null,
    resolveState: (row.resolve_state as string) || null,
...
export async function createAgent(input: CreateAgentInput): Promise<Agent> {
  ...
  const result = await pool
    ...
    .input("claimState", sql.NVarChar(50), input.claimState || null)
    .input("workingState", sql.NVarChar(50), input.workingState || null)
    .input("resolveState", sql.NVarChar(50), input.resolveState || null)
...
export async function updateAgent(
  ...
  const result = await pool
    ...
    .input("claimState", sql.NVarChar(50), input.claimState !== undefined ? input.claimState : existing.claimState)
    .input("workingState", sql.NVarChar(50), input.workingState !== undefined ? input.workingState : existing.workingState)
    .input("resolveState", sql.NVarChar(50), input.resolveState !== undefined ? input.resolveState : existing.resolveState)
    .query(`
      UPDATE agents
      SET name = @name,
